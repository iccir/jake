...
FileBase = new (function () {
  ...
  this.updateModTime = function () {
    ...
    if (jake.program.opts['use-fs-watch']) {
      if (!this.watcher || !this.modTime || this.watcherChanged) {
      ...
      if (!this.watcher) {
        this.watcher = fs.watch(this.name, { persistent: false }, function(ev, filename) {
          if (ev == "change") {
